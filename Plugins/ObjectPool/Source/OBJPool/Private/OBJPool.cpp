...
void UObjectPool::FlushObjectPool() 
{
	...
	for (int32 ID = Pool.Num()-1; ID >= 0; --ID) 
	{
		if (!Pool[ID]->IsValidLowLevelFast() || Pool[ID]->IsPendingKill()) 
		{
			Pool.RemoveAt(ID,1,false);
		}
	}///
}
...
{
	//判断当前池是否有效
	if (!GetOwner()->IsValidLowLevelFast()||this->IsPendingKill()) 
	{
		PooledActor->Destroy(true,true); 
		return;
	}
	//判断对象是否有效
	if (!PooledActor->IsValidLowLevelFast()||PooledActor->IsPendingKill()) 
	{
		PooledActor->Destroy(true,true); 
		return;
	}
...
void UPawnPool::GetObjectsFromPool(TArray<APooledPawn*> &Spawned, TArray<APooledPawn*> &Inactive) {
	for (const auto &OBJ : Pool) 
	{
		if (!OBJ->IsValidLowLevelFast() || OBJ->IsPendingKill()) 
		{ 
			continue; 
		}
		if (OBJ->Spawned)
